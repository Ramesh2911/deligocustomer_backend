...
// const todistance = Number(vendorLocation.distance_km.toFixed(2)); 
  ...
  try {
    ...
    const [vendorRows] = await con.execute(
      `SELECT 
       ...
       FROM hr_users u
       JOIN hr_addresses a ON a.user_id = ? AND a.is_active = 1
       WHERE u.id = ?`,
      [userid, vendorId]
    );
    ...
    const ptval = cartSumRows[0].ptval;
    const deliveryfee =
      settings.delivery_rider_charges +
      todistance * settings.delivery_rider_per_km_price;
    const totalAmount = ptval + deliveryfee;
    ...
    // ✅ 9. Update order after payment completion (simulate success)
    // ⚠️ In production, you should confirm via Stripe webhook
    await con.execute(
      `UPDATE hr_order 
       SET stripe_customer_id = ?, stripe_payment_id = ?, stripe_payment_method = ?, payment_status = ?
       WHERE oid = ?`,
    ...
    // ✅ 10. Only after payment success → insert notification
    await con.execute(
      `INSERT INTO hr_notification (user_id, title, readwrite, create_time, modified_time)
       VALUES (?, ?, ?, NOW(), NOW())`,
      [vendorId, "Customer new order request sent", 0]
    );
